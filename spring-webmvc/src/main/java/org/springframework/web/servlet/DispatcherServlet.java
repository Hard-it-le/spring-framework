...
public class DispatcherServlet extends FrameworkServlet {
	...
	protected void doDispatch(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			try {
				...
				/**
				 * 2、构造出了【目标方法+拦截器整个链路】决定使用哪个Handler处理当前请求 Determine handler for the current request.
				 *  取得处理当前请求的Controller（Handler）处理器
				 *  返回HandlerExecutionChain请求处理链接对象
				 *
				 *  该对象封装的是Handler和Inteceptor
				 *
				 */
				...
				/**
				 * 3、适配器怎么找的、 Determine handler adapter for the current request.
				 * 获取处理请求的处理适配器HandlerAdapter
				 */
	...
	private void processDispatchResult(HttpServletRequest request, HttpServletResponse response,
		...
		// 动态策略。 Did the handler return a view to render?
		// 为啥？@ResponseBody（提前在解析返回值的时候，就已经把数据写出去了，所以这一步就没有了）
		if (mv != null && !mv.wasCleared()) {
			//渲染ModeAndView，来解析模型和视图；最终决定响应效果
	...
	protected HandlerExecutionChain getHandler(HttpServletRequest request) throws Exception {
		//如果handlerMappings集合 为空，则直接返回为空
		if (this.handlerMappings != null) {
			//获取处理器的映射
			for (HandlerMapping mapping : this.handlerMappings) {
				//查询mapping中的handler是否存在
	...
	protected void render(ModelAndView mv, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		View view; //视图
		//适配器执行完目标方法以后返回的ModelAndView对象里面有  index.jsp
	...
	protected View resolveViewName(String viewName, @Nullable Map<String, Object> model,
		...
		if (this.viewResolvers != null) {
			for (ViewResolver viewResolver : this.viewResolvers) {
				...
				if (view != null) {
					//即使一个返回值满足多种视图解析规则，排在前面的视图解析器解析成功就直接返回
					return view;
				}
			}
		}
